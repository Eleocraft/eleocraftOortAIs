...
// Prediction stuff
const TURRET_BULLET_SPEED: f64 = 1000.0; // m/s
const RAILGUN_BULLET_SPEED: f64 = 4000.0; // m/s
const PASSES: i32 = 6;
...
impl Ship {
    ...
    fn frigate_tick(&mut self) {
        if self.scans_to_do <= 0 {
            ...
            debug!("Scanning...");
            if let Some(contact) = scan() {
                if contact.class != Class::Missile && angle_diff(heading(), (contact.position - position()).angle()).abs() < self.closest_ship_angle.abs() {
    ...
    fn frigate_targeting(&mut self, target: Vec2, target_velocity: Vec2) {
        ...
        // -- turning and aiming --
        let lead_position = Self::lead(target, target_velocity, target_acceleration, RAILGUN_BULLET_SPEED, position());
        ...
        let right_turret_lead_position = Self::lead(target, target_velocity, target_acceleration, TURRET_BULLET_SPEED, right_turret_position);
        let left_turret_lead_position = Self::lead(target, target_velocity, target_acceleration, TURRET_BULLET_SPEED, left_turret_position);
        ...
        if turning_angle.abs() < MISSILE_FIRE_ANGLE || (target - position()).length() < MISSILE_FIRE_DIST {
            fire(3);
        }
        ...
        draw_polygon(right_turret_lead_position, ENEMY_SIZE / 2.0, 10, 30.0, 0x00ff00); // right turret lead indicator
        draw_polygon(left_turret_lead_position, ENEMY_SIZE / 2.0, 10, 30.0, 0x00ff00); // left turret lead indicator
    }
