...
// Crusing mode Stuff
const VELOCITY_MULTIPLIER_FOR_DF: f64 = 15.0;
const C_TURNING_RATE_MULT: f64 = 10.0;
...
impl Ship {
    ...
    pub fn fighter_targeting(&mut self, target: Vec2, target_velocity: Vec2) {
        ...
        debug!("{}", -relative_directional_velocity*VELOCITY_MULTIPLIER_FOR_DF - relative_position.length());
        // main argument (use dogfight mode if the two spaceships are close or closing in)
        if -relative_directional_velocity*VELOCITY_MULTIPLIER_FOR_DF - relative_position.length() > -BULLET_SPEED {
            ...
            // crusing mode (get back to target) using proportional navigation
            let acceleration = Self::calculate_prop_nav_acceleration(target, target_velocity);
            debug!("Crusing...");
            debug!("acceleration: {:.2}", acceleration.length());
            turn(C_TURNING_RATE_MULT * angle_diff(heading(), acceleration.angle()));
            accelerate(acceleration);
        }
        // fire missiles if looking in the right direction or close enough the missile can lock on
        if angle_diff(heading(), relative_position.angle()) < MISSILE_FIRE_ANGLE || relative_position.length() < MISSILE_FIRE_DIST {
            fire(1);
        }
    }
    pub fn dogfight_mode(&self, target: Vec2, target_velocity: Vec2, target_acceleration: Vec2) {
        ...
        // -- turning and aiming --
        let lead_position = Self::lead(target, target_velocity, target_acceleration, BULLET_SPEED, PASSES);
    ...
    pub fn calculate_prop_nav_acceleration(target: Vec2, target_velocity: Vec2) -> Vec2 {
        ...
        debug!("perpendicular acceleration: {}", perpendicular_acceleration);
        // a = los * a_los + n * a_n
    ...
    pub fn get_bullet_time(target: Vec2, bullet_speed: f64) -> f64 { // Calculate the actual speed of bullets (taking into account ship velocity)
        return (target - position()).length() / (bullet_speed + velocity().dot((target - position()).normalize()));
    }

    pub fn lead(target: Vec2, target_velocity : Vec2, target_acceleration: Vec2, bullet_speed: f64,  passes: i32) -> Vec2 {
        let mut target_projected = target;
        for _i in 0..passes { // repeatedly projecting with the new distance
            let t = Self::get_bullet_time(target_projected, bullet_speed); // calculating time to encounter with given speed
            target_projected = target + target_velocity * t + 0.5 * target_acceleration * t * t; // Calculating lead to target
        }
        let self_projected_relative = velocity() * Self::get_bullet_time(target_projected, bullet_speed);
